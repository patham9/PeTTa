...
maybe_call(F, Args, Out) :- nonvar(F), atom(F), fun(F) 
...
% ---------- internals ----------
build_pycall_term_(Base, Name, Args, Term) :- Call =.. [Name|Args], Term =.. [(:), Base, Call].

resolve_base_last_(Spec, Base, Name) :- ( string(Spec) -> atom_string(A,Spec) ; A=Spec ),
...
walk_attrs_(Obj0, [Attr|Rest], Obj) :- to_atomish_(Attr, A),
