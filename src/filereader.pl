...
process_metta_string(S, Results, Space) :- string_codes(S, Cs),
                                           strip(Cs, 0, Codes),
                                           phrase(top_forms(Forms, 1, 1), Codes),
                                           maplist(emit_source_debug, Forms),
...
process_form(Space, parsed(function, Index, Line, FormStr, Term, Env), []) :- add_sexp(Space, Term),
                                                            ...
                                                            ( legacy_verbose_enabled(compile)
...
top_forms([],_,_) --> blanks, eos.
top_forms([Term|Fs], LC0, Index0) --> newlines(LC0, LC1),
                                      ...
                                      { string_codes(FormStr, Cs),
                                        Term = source_form(Tag, Index0, LC1, FormStr),
                                        Index1 is Index0 + 1 },
                                      top_forms(Fs, LC2, Index1).
